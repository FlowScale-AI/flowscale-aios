...
if (process.platform !== "darwin") {
  if (isDev) {
    app.setAsDefaultProtocolClient("flowscaleaios", process.execPath, [
      __filename,
    ]);
  } else {
  ...
  if (process.platform === "linux") {
    ...
    try {
      ...
      execSync(
        `xdg-mime default flowscale-aios.desktop x-scheme-handler/flowscaleaios`,
      );
    } catch (err) {
...
function startNextServer(port: number): void {
  ...
  log.info(
    "[server] Starting Next.js standalone server:",
    serverScript,
    "on port",
    port,
  );
...
app.whenReady().then(async () => {
  ...
  const port = isDev
    ? AIOS_PORT_START
    : await findAvailablePort(AIOS_PORT_START);
