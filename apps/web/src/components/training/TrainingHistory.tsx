    ...
    <div>
      ...
      <div className="flex flex-col gap-2">
        {executions.map((exec) => {
          ...
          const outputs = exec.outputsJson ? JSON.parse(exec.outputsJson) as Array<{ path?: string; filename?: string }> : null
          ...
          return (
            <div
              key={exec.id}
              className="flex items-center gap-3 rounded-lg border border-white/5 bg-zinc-900/40 px-3 py-2.5"
            >
              {/* Status icon */}
