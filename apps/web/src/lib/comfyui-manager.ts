...
function findPythonExec(comfyPath: string): string {
  ...
  const candidates = isWin
    ? [
        // Windows portable installs: python_embeded is in the parent directory
        // (ComfyUI_windows_portable/python_embeded/python.exe relative to ComfyUI_windows_portable/ComfyUI/main.py)
...
export function getInstanceStatus(instanceId: string): InstanceStatusResult {
  ...
  if (isProcessAlive(pid))
    return { id: instanceId, alive: true, pid, port, device, label };
    ...
    for (const line of raw.trim().split("\n")) {
      ...
      try {
        ...
        if (device === "cpu") {
          ...
          const envKey = device.startsWith("rocm:")
            ? "HIP_VISIBLE_DEVICES"
...
export async function startInstance(
  ...
  if (await isPortInUse(config.port)) {
    throw new Error(
      `Port ${config.port} is already in use — a ComfyUI instance may already be running for ${config.label}`,
    );
  }

  // comfyManagedPath is the root of the installation (e.g. portable root or GitHub clone)
  // comfyUIPath is where main.py lives (e.g. portable/ComfyUI or GitHub clone itself)
  const comfyUIPath = getComfyUIPath();
  const comfyManagedPath = getComfyManagedPath();

  // Use comfyUIPath for main.py location; fall back to comfyManagedPath for backward compat
  const comfyPath = comfyUIPath || comfyManagedPath;
  if (!comfyPath)
    throw new Error(
      "ComfyUI path not configured. Please complete the setup first.",
    );
  if (!existsSync(comfyPath))
    throw new Error(`ComfyUI directory not found: ${comfyPath}`);

  const mainPy = path.join(comfyPath, "main.py");
  if (!existsSync(mainPy)) {
    throw new Error(
      `ComfyUI main.py not found at: ${mainPy}. Is this a valid ComfyUI installation?`,
    );
  }
  ...
  child.stdout?.on("data", () => {
    /* consumed to avoid back-pressure */
  });
...
export async function startAll(): Promise<
  Array<{ id: string; port: number; pid: number }>
> {
...
export async function startComfyUI(): Promise<{ port: number; pid: number }> {
  ...
  if (instances.length === 0)
    throw new Error("No ComfyUI instances configured");
