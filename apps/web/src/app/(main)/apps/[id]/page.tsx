...
import {
  FileUploadInput,
  inferInputUploadKind,
} from "@/components/FileUploadInput";
import {
  LocalInferenceSetup,
  useInferenceStatus,
} from "@/components/LocalInferenceSetup";
...
function inferOutputKind(
  nodeType: string,
): "image" | "video" | "audio" | "model" | "text" | "file" {
  if (
    [
      "FSSaveImage",
      "SaveImage",
    ...
    return "image";
  if (["FSSaveVideo", "VHS_VideoCombine", "SaveVideo"].includes(nodeType))
    return "video";
  if (["FSSaveAudio", "SaveAudio", "PreviewAudio"].includes(nodeType))
    return "audio";
  if (
    [
      "FSSave3D",
    ...
    /Save.*3[Dd]|3[Dd].*Save|GLB|GLTF|Mesh/i.test(nodeType)
  )
    return "model";
...
function OutputLoadingPlaceholder({
  kind,
}: {
  kind: "image" | "video" | "audio" | "model" | "text" | "file";
}) {
  if (kind === "text") {
    return (
...
function ModelPreview({ src, filename }: { src: string; filename: string }) {
  ...
  useEffect(() => {
    if (viewable) loadMV().then(() => setReady(true));
  }, [viewable]);
  if (viewable && ready) {
    return (
      // @ts-ignore
...
type OutputItem =
  | {
      kind: "image" | "video" | "audio" | "model" | "file";
      filename: string;
      path: string;
    }
...
function resolveOutputUrl(
  out: Exclude<OutputItem, { kind: "text" }>,
  comfyPort?: number | null,
): string {
  if (out.path.startsWith("/")) return out.path;
  const subfolder = out.path.includes("/")
    ? out.path.substring(0, out.path.lastIndexOf("/"))
    : "";
...
function OutputLightbox({
  ...
  return (
    ...
    >
      <div
        className="relative flex flex-col bg-zinc-900 rounded-xl overflow-hidden max-w-[90vw] max-h-[90vh] shadow-2xl"
...
function OutputGrid({
  ...
  const cardClass =
    "group flex flex-col rounded-xl overflow-hidden border border-white/5 bg-zinc-900/50 hover:border-emerald-500/30 transition-all duration-200 cursor-pointer";
        ...
        {outputs.map((out, i) => {
          if (out.kind === "text") {
            return (
              <div
                key={i}
                className="col-span-2 sm:col-span-3 rounded-xl border border-white/5 bg-zinc-900/50 px-4 py-3"
              >
                <p className="text-sm text-zinc-300 whitespace-pre-wrap font-mono-custom">
                  {out.text}
                </p>
              </div>
            );
  ...
  if (field.paramType === "boolean") {
    return (
      <div className="flex items-center gap-3">
        ...
        />
        <label
          htmlFor={`${field.nodeId}__${field.paramName}`}
...
function ExecutionHistoryItem({
  ...
  const outputs = exec.outputsJson
    ? (
        JSON.parse(exec.outputsJson) as {
          kind: string;
          filename?: string;
          path?: string;
        }[]
      ).filter(
        (o): o is { kind: string; filename: string; path: string } =>
          !!o.filename,
      )
    : [];

  const inputs: Record<string, unknown> = exec.inputsJson
    ? JSON.parse(exec.inputsJson)
    : {};
  ...
  return (
    <div className="border border-white/5 rounded-lg p-3 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        {exec.status === "running" && <LottieSpinner size={14} />}
          ...
          {outputs.map((out) => (
            <a
              ...
              href={
                out?.path?.startsWith("/")
                  ? out.path
                  : `/api/comfy/output?path=${encodeURIComponent(out.path)}`
              }
      ...
      {exec.seed !== null && (
        <span className="text-[11px] text-zinc-600 font-mono-custom">
          seed: {exec.seed}
        </span>
      )}
    </div>
...
function NodeJsTab({
  ...
  const snippet = `import { createClient, login } from '@flowscale/sdk'
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        <p className="text-xs text-zinc-500 mb-2">
          Run — values reflect the current form inputs
        </p>
...
function HttpTab({
  ...
  const runSnippet = `const BASE = 'http://localhost:14173'
    ...
    <div className="flex flex-col gap-5">
      <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
        <p className="text-xs text-amber-400 leading-relaxed">
          Update <code className="bg-amber-500/10 px-1 rounded">BASE</code> if
          your AIOS instance is not running locally. Session tokens are obtained
          by logging in via{" "}
      ...
      <div>
        <p className="text-xs text-zinc-500 mb-2">
          Run &amp; poll — inputs reflect current form values
        </p>
...
function ServerLogsPanel() {
  ...
  if (logs.length === 0)
    return <p className="text-xs text-zinc-600 pt-2">No server logs yet.</p>;

  return (
    <div
      ref={containerRef}
      onScroll={handleScroll}
      className="h-full overflow-y-auto font-mono text-[11px] text-zinc-400 leading-relaxed"
    >
      {logs.map((line, i) => (
...
function BottomTabs({
  ...
  useEffect(() => {
    if (
      tool.engine === "api" &&
      (inferenceStatus === "starting" || inferenceStatus === "running")
    ) {
      setTab("logs");
    }
  ...
  return (
    <div className="h-64 flex flex-col border-t border-white/5">
      ...
      <div className="flex-1 overflow-hidden px-4 pb-4 pt-2">
        ...
        {tab === "logs" &&
          tool.engine !== "api" &&
          (effectiveComfyPort ?? tool.comfyPort) && (
            <ComfyLogsPanel
              port={(effectiveComfyPort ?? tool.comfyPort)!}
              instanceLabel={comfyInstanceLabel}
            />
          )}
        {tab === "logs" &&
          tool.engine !== "api" &&
          !(effectiveComfyPort ?? tool.comfyPort) && (
            <p className="text-xs text-zinc-600 pt-2">
              No ComfyUI instance configured.
            </p>
          )}
      </div>
    </div>
...
export default function ToolPage() {
  ...
  const allSchema: WorkflowIO[] = tool?.schemaJson
  ...
  const expectedOutputKinds: Array<
    "image" | "video" | "audio" | "model" | "text" | "file"
  > = allSchema
    .filter((f) => !f.isInput && f.enabled !== false)
    .map((f) => inferOutputKind(f.nodeType));
  ...
  const [selectedComfyPort, setSelectedComfyPort] = useState<
    number | "auto" | null
  >(null);
  ...
  const comfyInstanceLabel = effectiveComfyPort
    ? (comfyInstances.find((i) => i.port === effectiveComfyPort)?.label ??
      `:${effectiveComfyPort}`)
    : undefined;

  // ── GPU/device selection for API tools ────────────────────────────────────────
  const { data: gpuData } = useQuery<{
    instances: Array<{ id: string; device: string; label: string }>;
  }>({
      ...
      const finish = async () => {
        ...
        try {
          const histRes = await fetch(
            `/api/comfy/${run.comfyPort}/history/${run.promptId}`,
          );
          if (histRes.ok) {
            ...
            for (const nodeOut of Object.values(entry?.outputs ?? {})) {
              for (const f of nodeOut.images ?? [])
                items.push({
                  kind: inferKind(f.filename),
                  filename: f.filename,
                  path: `${f.subfolder ? f.subfolder + "/" : ""}${f.filename}`,
                });
              for (const f of nodeOut.gifs ?? [])
                items.push({
                  kind: inferKind(f.filename),
                  filename: f.filename,
                  path: `${f.subfolder ? f.subfolder + "/" : ""}${f.filename}`,
                });
              for (const f of nodeOut.videos ?? [])
                items.push({
                  kind: "video",
                  filename: f.filename,
                  path: `${f.subfolder ? f.subfolder + "/" : ""}${f.filename}`,
                });
              for (const f of nodeOut.audio ?? [])
                items.push({
                  kind: "audio",
                  filename: f.filename,
                  path: `${f.subfolder ? f.subfolder + "/" : ""}${f.filename}`,
              ...
              for (const t of [
                ...
                if (typeof t === "string" && t.trim()) {
                  const k = inferKind(t);
                  if (k !== "file")
                    items.push({ kind: k, filename: t, path: t });
            ...
            await fetch(`/api/executions/${run.executionId}`, {
              ...
              body: JSON.stringify({
                status:
                  entry?.status?.status_str === "error" ? "error" : "completed",
                ...
                {leftTab === "form" &&
                  (schema.length === 0 ? (
                    <p className="text-sm text-zinc-600">
                      No configurable inputs detected.
                    </p>
                  ) : (
