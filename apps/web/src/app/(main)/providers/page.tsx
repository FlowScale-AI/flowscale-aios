...
const PROVIDER_DESCRIPTIONS: Record<string, string> = {
  ...
  replicate: 'Run open-source models in the cloud via Replicate with a single API call.',
  openrouter: 'Access hundreds of LLMs through a single API — route to the best model for your needs.',
  huggingface: 'Access the HuggingFace Inference API to run models from the Hub.',
}
...
function ProviderCard({ provider }: { provider: ProviderStatus }) {
  ...
  const removeMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/providers/${provider.name}`, { method: 'DELETE' })
    ...
    <div className={[
        ...
        <div className="flex items-center gap-3">
          <div className="size-9 rounded-lg border border-white/10 bg-zinc-800 flex items-center justify-center overflow-hidden shrink-0">
            {PROVIDER_ICONS[provider.name] ? (
              <img src={PROVIDER_ICONS[provider.name]} alt={provider.label} className="size-5 object-contain" />
            ) : (
              <span className="text-xs font-bold text-zinc-400">{provider.label[0]}</span>
            )}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-zinc-200">{provider.label}</span>
              {provider.configured ? (
            ...
            <p className="text-xs text-zinc-500 mt-0.5">{PROVIDER_DESCRIPTIONS[provider.name]}</p>
          </div>
        </div>
        <a
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            placeholder={provider.configured ? '••••••••••••••••' : 'Paste API key…'}
            value={keyInput}
      ...
      {provider.configured && (
        <div className="flex items-center gap-3 mt-3">
          ...
          >
            {testState === 'testing' && <CircleNotch size={12} className="animate-spin" />}
            {testState === 'ok' && <CheckCircle size={12} className="text-emerald-400" />}
            {testState === 'error' && <XCircle size={12} className="text-red-400" />}
            {testState === 'idle' && null}
            {testState === 'idle' ? 'Test Connection' : testState === 'testing' ? 'Testing…' : testState === 'ok' ? 'Connected' : 'Failed'}
          </button>
  ...
  return (
    <PageTransition className="h-full flex flex-col bg-[var(--color-background)] overflow-y-auto">
      ...
      <div className="px-8 py-6 border-b border-white/5 shrink-0">
        <h1 className="font-tech text-xl font-semibold text-zinc-100">Providers</h1>
        <p className="text-sm text-zinc-500 mt-0.5">Configure your AI inference sources</p>
      </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          {/* Local Inference */}
          <section>
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-4">Local Inference</h2>
            <div className="p-5 rounded-xl border border-white/10 bg-[var(--color-background-panel)]">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-zinc-200">ComfyUI</span>
                    ...
                    {comfyInstances.length > 0 ? (
                      ...
                      <p className="text-xs text-zinc-600 mt-0.5">Start ComfyUI to connect</p>
                    )}
                  </div>
                </div>
                <Link
              ...
              <div className="border-t border-white/5 pt-4">
                <label className="block text-xs font-medium text-zinc-500 mb-1.5">
                  Installation path
                  <span className="ml-1 text-zinc-600 font-normal">(required for model downloads)</span>
                </label>
                {savedPath && (
                ...
                <p className="text-[11px] text-zinc-600 mt-1.5">
                  The root directory of your ComfyUI install. Models will be downloaded into{' '}
                  <span className="font-mono-custom">models/</span> subdirectories.
                </p>
              </div>
            </div>
          </section>
          ...
          {/* Cloud Providers */}
          <section>
            <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-4">Cloud Providers</h2>
            {isLoading ? (
            ...
            <p className="text-xs text-zinc-600 mt-4">
              ...
              <span className="font-mono-custom">~/.flowscale/aios/provider-keys.json</span> and never sent to FlowScale servers.
            </p>
          </section>
