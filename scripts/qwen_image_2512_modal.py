"""
Qwen-Image-2512 — Modal.com deployment.

Deploy once:
    modal deploy scripts/qwen_image_2512_modal.py
...
GET /health returns { "status": "ok", "model": "Qwen/Qwen-Image-2512" }.
"""
...
# ── Volume ───────────────────────────────────────────────────────────────────
model_volume = modal.Volume.from_name("qwen-image-2512-weights", create_if_missing=True)
...
# ── App ───────────────────────────────────────────────────────────────────────
app = modal.App("qwen-image-2512", image=image)
...
class QwenImage2512:
    ...
    def load(self):
        ...
        model_path = os.path.join(MODEL_DIR, "qwen-image-2512")
        if not os.path.exists(model_path) or not os.listdir(model_path):
            print("Downloading Qwen-Image-2512 weights...")
